...
2. **스코어보드 OCR 자동 인식**: 게임 내 스코어보드 스크린샷을 업로드하면 AI가 KDA, 골드, CS 등 주요 지표를 자동으로 추출하여 샌드박스에 즉시 반영합니다.
3. **다중 ML 모델 승률 예측**: XGBoost, Random Forest, Logistic Regression 모델을 선택 및 교체하여, 각 알고리즘이 예측하는 실시간 승률 변화 및 최종 유력 팀 결과를 시각화합니다.
4. **상호 대화형 자동 밸런싱**: 블루팀 킬 증가 시 레드팀 데스 자동 동기화, CS 증가 시 획득 골드 자동 비례 증가 등 정형 데이터 샌드박스의 편의성을 제공합니다.
5. **모델 벤치마크 및 지표 비교**: 각 머신러닝 알고리즘의 Accuracy, F1-Score, ROC-AUC 메트릭을 카드 인터페이스와 스케일바로 투명하게 시각화하고 즉석에서 모델 재학습을 트리거할 수 있습니다.
