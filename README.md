...
```text
.
├── 📁 models/                  # 학습 완료된 모델 파라미터 및 메타 데이터 저장소 (Git 제외)
...
├── 📄 app.py                   # Flask 메인 애플리케이션 (Riot API 연동 및 챔피언 시너지/조합 계산 내장)
...
* **`app.py`**: 서버의 심장부로 클라이언트와의 HTTP 통신을 담당합니다. 챔피언 및 조합 시너지 평가 코드 모듈이 인라인 내장되어 있어 단일 파일로서 완벽히 작동합니다.
